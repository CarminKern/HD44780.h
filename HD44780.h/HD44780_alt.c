...
int HD44780_datasend(char RS, char RW, char D7, char D6, char D5, char D4, char D3, char D2, char D1, char D0) {
	...
	/* Funktionsbeschreibung
		Diese Funktion sendet die Daten RS, RW, D7, D6, D5, D4, D3, D2, D1, D0 an die HD44780.
		Die Daten werden mittels GPIO Programm (GPIO_H) gesendet.
		Parameter überprüfen:
			Die übergebenden Parameter werden am vorher überprüft,
			wenn die Parameter nicht 0, 1 oder 'X' sind wird die Funktion abgebrochen und der Wert 1 zurückgegeben.
		Senden an HD44780:
			Je nach Modus (4/8 Bit) werden die einzelnen Pins eingestellt und an die HD44780 gesendet.
			Die Pins werden nicht verstellt wenn der jeweile Parameter 'X' ist oder
			wenn der Pin bereits den einzustellenden Parameter entspricht.
			Je nach M
	...
	*/
	...
	// Parameter überprüfen
		...
		// Abbruch wenn die Parameter nicht 0/1 oder 'X' sind
			/*FEHLERMELDUNG -> Parameter entsprechen nicht 0 / 1 / X */
			return datasend_error = 1;
	}

	// Senden an HD44780 
		// Die Einstellungen werden nicht verstellt, wenn der Parameter 'X' (Dont Care) ist oder der Parameter dem letzten eingestellten Wert entspricht)
			...
			// 4 Bit Modus
					...
					// Einstellen von D3 auf D7
						if (datasend.D3 != lastdata.D7 && datasend.D3 != 88) {
							GPIO_SET("D3", pinset.D7, datasend.D3);
